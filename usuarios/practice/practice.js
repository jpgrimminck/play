(() => {
  ...
  function buildSongsIndexUrl(userId) {
    const configuredBase = typeof window.USUARIOS_SONGS_INDEX_URL === 'string' ? window.USUARIOS_SONGS_INDEX_URL.trim() : '';
    let baseUrl = null;

    if (configuredBase) {
      baseUrl = new URL(configuredBase, window.location.href);
    } else {
      const path = window.location.pathname || '';
      const runningInsideUsuariosPractice = path.includes('/usuarios/practice');
      baseUrl = runningInsideUsuariosPractice
        ? new URL('../songs/index.html', window.location.href)
        : new URL('usuarios/songs/index.html', window.location.href);
    }
  ...
  function setState3(id, name = null, animate = false, options = {}) {
    ...
    btn.onclick = async () => {
      ...
      window.location.href = buildSongsIndexUrl(id);
    };
    if (animate) {
      setTimeout(() => {
        const progressDiv = btn.querySelector('div');
